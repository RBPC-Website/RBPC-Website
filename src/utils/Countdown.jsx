...
export default function Countdown() {
    ...
    useEffect(() => {
        document.querySelector('header').style.top = showBanner ? '30px' : '0px';
    }, [showBanner]);

    return (
        <>
        {showBanner && (
            <div id="sticky-banner" tabindex="-1" className="fixed top-0 left-0 z-50 flex justify-between w-full h-[30px] p-4 border-b border-gray-200 bg-green-100 text-black-100">
                <div className="flex items-center mx-auto">
                    <p className="flex items-center text-[14px] text-black-100">
                        ...
                        <p><span className="font-medium">{t(getTimeLeft(Date.now()).event)}</span> will start in <span className="font-medium">{timeLeft}</span></p>
                    </p>
                </div>
                <div className="flex items-center">
                    <button data-dismiss-target="#sticky-banner" type="button" onClick={() => {setShowBanner(false)}} className="flex-shrink-0 inline-flex justify-center items-center text-black-100 hover:bg-gray-100 hover:text-gray-900 rounded-lg text-sm p-1.5 dark:hover:bg-gray-600 dark:hover:text-white">
                        <svg aria-hidden="true" className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                    </button>
                </div>
            </div>
        )}
